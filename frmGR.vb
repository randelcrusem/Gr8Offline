Public Class frmGR
    ...
    Private Sub LoadGR(ByVal ID As String)
        ...
        query = " SELECT   TransID, GR_No, VCECode, DateGR, Type, ReceivedBy, ReceivedFrom, WHSE_From, WHSE_To, Remarks, ISNULL(GI_Ref,0) AS GI_Ref, Status " & _
                " FROM     tblGR " & _
                " WHERE    TransId = '" & ID & "' " & _
                " ORDER BY TransId "
        SQL.ReadQuery(query)
        ...
        Try
            ...
            For Each row As DataGridViewRow In dgvItemMaster.Rows
                If Not row.Cells(dgcQTY.Index).Value Is Nothing AndAlso Not row.Cells(chItemCode.Index).Value Is Nothing Then
                    ...
                    SaveINVTY("OUT", ModuleID, "GI", GI_ID, dtpDocDate.Value.Date, ItemCode, WHSEfrom, QTY, UOM, UnitCost)
                    ...
                    Try
                        ...
                        For Each row As DataGridViewRow In dgvItemMaster.Rows
                            If Not row.Cells(dgcQTY.Index).Value Is Nothing AndAlso Not row.Cells(chItemCode.Index).Value Is Nothing Then
                                ...
                                SaveINVTY("IN", ModuleID, "GI", GI_ID, dtpDocDate.Value.Date, ItemCode, WHSEfrom, QTY, UOM, UnitCost)
                                SaveINVTY("OUT", ModuleID, "GR", TransID, dtpDocDate.Value.Date, ItemCode, WHSEto, QTY, UOM, UnitCost)
                            End If
                        Next
