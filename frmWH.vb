Public Class frmWH
    ...
    Private Sub loadGroupField(ByVal GroupID As String, lblName As Label, cbName As ComboBox)
        lblName.Text = LoadGroupName("Warehouse", GroupID)
    ...
    Private Sub LoadGroupData(ByVal GroupID As String, cbName As ComboBox)
        Dim query As String
        query = " SELECT DISTINCT ISNULL(" & GroupID & ",'') AS GroupData FROM tblWarehouse WHERE  ISNULL(" & GroupID & ",'') <> '' AND Status ='Active'"
        SQL.ReadQuery(query)
        cbName.Items.Clear()
    ...
    Private Sub SaveRecord()
        Try
            ...
            insertSQL = " INSERT INTO " & _
                        " tblWarehouse(Code, Description, DefaultAccount, G1, G2, G3, G4, G5, WhoCreated) " & _
    ...
    Private Sub UpdateRecord()
        Try
            ...
            updateSQL = " UPDATE tblWarehouse " & _
            ...
            If txtCode.Text <> "" Then
                If MsgBox("Are you sure you want to delete this record?", MsgBoxStyle.Exclamation + MsgBoxStyle.YesNo, "JADE Message Alert") = MsgBoxResult.Yes Then
                    Try
                        activityStatus = True
                        Dim updateSQL As String
                        updateSQL = " UPDATE tblWarehouse SET Status ='Inactive', DateModified = GETDATE(), WhoModified = @WhoModified WHERE Code = @Code "
                        SQL.FlushParams()
                        Code = txtCode.Text
