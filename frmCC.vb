Public Class frmCC
    ...
    Private Sub loadGroupField(ByVal GroupID As String, lblName As Label, cbName As ComboBox)
        lblName.Text = LoadGroupName("Cost Center", GroupID)
    ...
    Private Sub LoadGroupData(ByVal GroupID As String, cbName As ComboBox)
        Dim query As String
        query = " SELECT DISTINCT ISNULL(" & GroupID & ",'') AS GroupData FROM tblCC WHERE  ISNULL(" & GroupID & ",'') <> ''"
        SQL.ReadQuery(query)
        cbName.Items.Clear()
    ...
    Private Sub LoadRecords() ' LOAD COST CENTER TABLE
        ...
        If Not dt Is Nothing Then
            ...
            query = " SELECT Code AS oldCode, Code, Description " & groupName & _
                    " FROM tblCC WHERE Status ='Active' "
            SQL.GetQuery(query)
    ...
    Private Sub tsbNew_Click(sender As System.Object, e As System.EventArgs) Handles tsbNew.Click
        If Not AllowAccess("CC_ADD") Then
            msgRestricted()
        Else
            Code = ""
            ClearText()
    ...
    Private Sub tsbSave_Click(sender As System.Object, e As System.EventArgs) Handles tsbSave.Click
        If Validated() Then
            If Code = "" Then
                If MsgBox("Saving New Cost Center, Click Yes to confirm", MsgBoxStyle.Information + MsgBoxStyle.YesNo, "JADE Message Alert") = MsgBoxResult.Yes Then
                    SaveRecord()
                    Msg("Record Saved Succesfully!", MsgBoxStyle.Information)
                    loadData()
                End If
            Else
                If MsgBox("Updating Cost Center, Click Yes to confirm", MsgBoxStyle.Information + MsgBoxStyle.YesNo, "JADE Message Alert") = MsgBoxResult.Yes Then
                    UpdateRecord()
    ...
    Private Sub SaveRecord()
        Try
            ...
            insertSQL = " INSERT INTO " & _
                        " tblCC(Code, Description, G1, G2, G3, G4, G5, WhoCreated) " & _
    ...
    Private Shadows Function Validated() As Boolean
        If txtCode.Text = "" Then
            Msg("Please enter cost center code!", MsgBoxStyle.Exclamation)
            Return False
        ElseIf txtDescription.Text = "" Then
            Msg("Please enter cost center name!", MsgBoxStyle.Exclamation)
            Return False
        Else
            Return True
        End If
    ...
    Private Sub tsbEdit_Click(sender As System.Object, e As System.EventArgs) Handles tsbEdit.Click
        If Not AllowAccess("CC_EDIT") Then
            msgRestricted()
        Else
            EnableControl(True)
    ...
    Private Sub tsbDelete_Click(sender As System.Object, e As System.EventArgs) Handles tsbDelete.Click
        If Not AllowAccess("CC_DEL") Then
            msgRestricted()
        Else
            If txtCode.Text <> "" Then
                If MsgBox("Are you sure you want to delete this record?", MsgBoxStyle.Exclamation + MsgBoxStyle.YesNo, "JADE Message Alert") = MsgBoxResult.Yes Then
                    Try
                        activityStatus = True
                        Dim updateSQL As String
                        updateSQL = " UPDATE tblCC SET Status ='Inactive', DateModified = GETDATE(), WhoModified = @WhoModified WHERE Code = @Code "
                        SQL.FlushParams()
                        Code = txtCode.Text
