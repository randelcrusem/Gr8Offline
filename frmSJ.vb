Public Class frmSJ
    ...
    Private Sub SaveSJ()
        Try
            ...
            insertSQL = " INSERT INTO " & _
                        " tblSJ (TransID, SJ_No, VCECode, BranchCode, BusinessCode, DateSJ, TotalAmount, Remarks, LN_Ref, TransAuto, WhoCreated, MemTransfer, Terms) " & _
                        " VALUES(@TransID, @SJ_No, @VCECode, @BranchCode, @BusinessCode, @DateSJ, @TotalAmount, @Remarks, @LN_Ref, @TransAuto, @WhoCreated, @MemTransfer, @Terms)"
            SQL.FlushParams()
            ...
            If LOAN_ID > 0 Then
                ...
                updateSQL = " UPDATE tblLoan SET Status ='Released', DateRelease = '" & dtpDocDate.Value.Date & "', RefType = 'SJ', RefTransID = '" & TransID & "' WHERE TransID = '" & LOAN_ID & "'"
            ...
            If strRefNo.Contains("LN:") Then
                ...
                For i As Integer = 0 To count
                    ...
                    If SQL.SQLDR.Read Then
                        Dim updateSQL As String
                        updateSQL = " UPDATE tblLoan SET Status ='Released', DateRelease = '" & dtpDocDate.Value.Date & "', RefType = 'SJ', RefTransID = '" & TransID & "' WHERE Loan_No = '" & strRefNo.Split("-")(i) & "'"
                        SQL.ExecNonQuery(updateSQL)
                    End If
                Next
            End If
        ...
        Finally
            RecordActivity(UserID, ModuleID, Me.Name.ToString, "INSERT", "SJ_No", txtTransNum.Text, BusinessType, IIf(interestBranchCode <> "", interestBranchCode, BranchCode), "", activityStatus)
            SQL.FlushParams()
        End Try
    End Sub

    Private Sub UpdateSJ()
        Try
            Dim insertSQL, updateSQL, deleteSQL As String
            activityStatus = True
            updateSQL = " UPDATE tblSJ " & _
                        " SET    SJ_No = @SJ_No, BranchCode = @BranchCode, BusinessCode = @BusinessCode,  VCECode = @VCECode, DateSJ = @DateSJ, Terms = @Terms," & _
            ...
            If strRefNo.Contains("LN:") Then
                ...
                For i As Integer = 0 To count
                    ...
                    If SQL.SQLDR.Read Then
                        ...
                        updateSQL1 = " UPDATE tblLoan SET Status ='Released', DateRelease = '" & dtpDocDate.Value.Date & "', RefType = 'SJ', RefTransID = '" & TransID & "' WHERE Loan_No = '" & strRefNo.Split("-")(i) & "'"
        ...
        Catch ex As Exception
            ...
            SaveError(ex.Message, ex.StackTrace, Me.Name.ToString, ModuleID)
        Finally
            RecordActivity(UserID, ModuleID, Me.Name.ToString, "UPDATE", "SJ_No", txtTransNum.Text, BusinessType, IIf(interestBranchCode <> "", interestBranchCode, BranchCode), "", activityStatus)
            SQL.FlushParams()
        End Try
    End Sub

    Private Sub LoadSJ(ByVal ID As String)
        Dim query, terms As String
        query = " SELECT  TransID, SJ_No, tblSJ.VCECode, VCEName, DateSJ, TotalAmount, Remarks, ISNULL(LN_Ref,0) as LN_Ref, tblSJ.Status, MemTransfer, tblSJ.Terms  " & _
                " FROM    tblSJ LEFT JOIN tblVCE_Master " & _
                " ON      tblSJ.VCECode = tblVCE_Master.VCECode " & _
                " WHERE   TransID ='" & ID & "' "
        SQL.ReadQuery(query)
        If SQL.SQLDR.Read Then
            ...
            txtTransNum.Text = SJNo
            txtVCECode.Text = SQL.SQLDR("VCECode").ToString
            txtVCEName.Text = SQL.SQLDR("VCEName").ToString
            disableEvent = True
            dtpDocDate.Value = SQL.SQLDR("DateSJ")
    ...
    Private Sub LoadAccountingEntry(ByVal TransID As Integer)
        Try
            ...
            query = " SELECT JE_No, VCECode, VCEName, AccntCode, AccntTitle, Particulars, RefNo," & _
                    ...
                    " FROM   View_GL  " & _
                    " WHERE  RefType ='SJ' AND RefTransID ='" & TransID & "' "
            SQL.ReadQuery(query)
            dgvEntry.Rows.Clear()
    ...
    Private Sub tsbEdit_Click(sender As System.Object, e As System.EventArgs) Handles tsbEdit.Click
        If Not AllowAccess("SJ_EDIT") Then
            msgRestricted()
        Else
            EnableControl(True)
    ...
    Private Sub tsbSave_Click(sender As System.Object, e As System.EventArgs) Handles tsbSave.Click
        ...
        ElseIf TransAuto = False AndAlso IfExist(txtTransNum.Text) And TransID = "" Then
            MsgBox("SJ" & " " & txtTransNum.Text & " already exist!", MsgBoxStyle.Exclamation)
        ElseIf TransID = "" Then
            ...
            If txtTransNum.Text <> "" Then
                If txtStatus.Text <> "Cancelled" AndAlso MsgBox("Are you sure you want to cancel this record?", MsgBoxStyle.Exclamation + MsgBoxStyle.YesNo, "JADE Message Alert") = MsgBoxResult.Yes Then
                    Try
                        activityStatus = True
                        Dim updateSQL As String
                        updateSQL = " UPDATE  tblSJ SET Status ='Cancelled' WHERE TransID = @TransID "
                        SQL.FlushParams()
                        SQL.AddParam("@TransID", TransID)
                        SQL.ExecNonQuery(updateSQL)

                        updateSQL = " UPDATE  tblJE_Header SET Status ='Cancelled' WHERE RefTransID = @RefTransID  AND RefType ='SJ' "
                        SQL.FlushParams()
                        SQL.AddParam("@RefTransID", TransID)
                        SQL.ExecNonQuery(updateSQL)
                        ...
                        insertSQL = " INSERT INTO tblJE_Details(JE_No, AccntCode, VCECode, Debit, Credit, Particulars, RefNo, LineNumber, OtherRef) " & _
                                    ...
                                    " WHERE JE_No IN (SELECT DISTINCT JE_No FROM tblJE_Header WHERE RefType ='SJ' AND RefTransID ='" & TransID & "') "
                        SQL.ExecNonQuery(insertSQL)
                    ...
                    Catch ex As Exception
                        ...
                        SaveError(ex.Message, ex.StackTrace, Me.Name.ToString, ModuleID)
                    Finally
                        RecordActivity(UserID, ModuleID, Me.Name.ToString, "CANCEL", "SJ_No", SJNo, BusinessType, IIf(interestBranchCode <> "", interestBranchCode, BranchCode), "", activityStatus)
                        SQL.FlushParams()
                    End Try
                ElseIf txtStatus.Text = "Cancelled" AndAlso MsgBox("Are you sure you want to un-cancel this record?", MsgBoxStyle.Exclamation + MsgBoxStyle.YesNo, "JADE Message Alert") = MsgBoxResult.Yes Then
                    Try
                        activityStatus = True
                        Dim updateSQL As String
                        updateSQL = " UPDATE  tblSJ SET Status ='Active' WHERE TransID = @TransID "
                        SQL.FlushParams()
                        SQL.AddParam("@TransID", TransID)
                        SQL.ExecNonQuery(updateSQL)

                        updateSQL = " UPDATE  tblJE_Header SET Status ='Saved' WHERE RefTransID = @RefTransID  AND RefType ='SJ' "
                        SQL.FlushParams()
                        SQL.AddParam("@RefTransID", TransID)
                        SQL.ExecNonQuery(updateSQL)
                        ...
                        insertSQL = " DELETE FROM tblJE_Details " & _
                                    " WHERE JE_No IN (SELECT DISTINCT JE_No FROM tblJE_Header WHERE RefType ='SJ' AND RefTransID ='" & TransID & "') AND LineNumber = 0 "
                        SQL.ExecNonQuery(insertSQL)
                        Msg("Record cancelled successfully", MsgBoxStyle.Information)


                        If LOAN_ID > 0 Then
                            updateSQL = " UPDATE tblLoan SET Status ='Released', DateRelease = '" & dtpDocDate.Value & "', RefType = 'SJ' RefTransID = '" & TransID & "' WHERE TransID = '" & LOAN_ID & "'"
                            SQL.ExecNonQuery(updateSQL)
                        End If
    ...
    Private Sub tsbPrevious_Click(sender As System.Object, e As System.EventArgs) Handles tsbPrevious.Click
        If SJNo <> "" Then
            Dim query As String
            query = "   SELECT Top 1 TransID FROM tblSJ  " & _
                    ...
                    " 	   ) " & _
                    "     )   " & _
                    "    AND SJ_No < '" & SJNo & "' ORDER BY SJ_No DESC  "
            SQL.ReadQuery(query)
            If SQL.SQLDR.Read Then
                TransID = SQL.SQLDR("TransID").ToString
                LoadSJ(TransID)
    ...
    Private Sub tsbNext_Click(sender As System.Object, e As System.EventArgs) Handles tsbNext.Click
        If SJNo <> "" Then
            ...
            query = "   SELECT Top 1 TransID FROM tblSJ  " & _
                    ...
                    " 	   ) " & _
                    "     )   " & _
                    "    AND SJ_No > '" & SJNo & "' ORDER BY SJ_No ASC  "
            SQL.ReadQuery(query)
            If SQL.SQLDR.Read Then
                TransID = SQL.SQLDR("TransID").ToString
                LoadSJ(TransID)
    ...
    Private Sub tsbDelete_Click(sender As System.Object, e As System.EventArgs) Handles tsbDelete.Click
        If MsgBox("Are you sure you want to delete this transaction?", MsgBoxStyle.YesNo + MsgBoxStyle.DefaultButton2, "Delete") = MsgBoxResult.Yes Then
            ...
            deleteSQL = "DELETE FROM tblJE_Header WHERE RefType = 'SJ' AND RefTransID = '" & TransID & "'"
            SQL.ExecNonQuery(deleteSQL)
    ...
    Private Sub frmSJ_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        Try
