Public Class frmSP
    ...
    Dim ColumnPK As String = "SP_No"
    Dim ColumnID As String = "TransID"
    Dim DBTable As String = "tblSP"
    ...
    Private Sub tsbNew_Click(sender As Object, e As EventArgs) Handles tsbNew.Click
        If Not AllowAccess("SP_ADD") Then
            msgRestricted()
        Else
            ClearText()
            TransID = ""
    ...
    Private Sub tsbEdit_Click(sender As Object, e As EventArgs) Handles tsbEdit.Click
        If Not AllowAccess("SP_EDIT") Then
            msgRestricted()
        Else
            EnableControl(True)
    ...
    Private Sub tsbSave_Click(sender As Object, e As EventArgs) Handles tsbSave.Click
        ...
        ElseIf TransID = "" Then
            If MsgBox("Saving New Record, Click Yes to confirm", MsgBoxStyle.Information + MsgBoxStyle.YesNo, "JADE Message Alert") = MsgBoxResult.Yes Then
                ...
                SPNo = GenerateTransNum(TransAuto, ModuleID, ColumnPK, DBTable)
                txtTransNum.Text = SPNo
                SaveSP()
                Msg("Record Saved Succesfully!", MsgBoxStyle.Information)
                SPNo = txtTransNum.Text
                LoadSP(TransID)
            End If
        Else
    ...
    Private Sub UpdateSP()
        Try
            ...
            updateSQL = " UPDATE tblSP" & _
                        " SET    SP_No = @SP_No, DateSP = @DateSP, VCECode = @VCECode, VCEName = @VCEName, SignatoryName = @SignatoryName, " & _
            ...
            SQL.AddParam("@SP_No", SPNo)
            SQL.AddParam("@DateSP", dtpDocDate.Value.Date)
            ...
            SQL.AddParam("@Area", cbArea.Text)
            SQL.AddParam("@Region", cbRegion.Text)
            SQL.AddParam("@Province", cbProvince.Text)
            SQL.AddParam("@City", cbCity.Text)
            ...
            deleteSQL = " DELETE FROM tblSP_Progress WHERE TransID = @TransID "
            SQL.FlushParams()
            SQL.AddParam("@TransID", TransID)
                ...
                For Each row As DataGridViewRow In dgvProgress.Rows
                    If Not row.Cells(dgcTermRemarks.Index).Value Is Nothing AndAlso IsNumeric(row.Cells(dgcTermPercent.Index).Value) Then
                        insertSQL = " INSERT INTO " & _
                                    " tblSP_Progress(TransID, Progress, Remarks, WhoCreated) " & _
            ...
            deleteSQL = " DELETE FROM tblSP_Details WHERE TransID = @TransID "
            SQL.FlushParams()
            SQL.AddParam("@TransID", TransID)
            SQL.ExecNonQuery(deleteSQL)
            ...
            For Each row As DataGridViewRow In dgvSoftware.Rows
                If Not row.Cells(dgcCode.Index).Value Is Nothing AndAlso Not row.Cells(dgcSoftPrice.Index).Value Is Nothing Then
                    insertSQL = " INSERT INTO " & _
                                " tblSP_Details(TransId, Type, ItemCode, Description, UOM, QTY, UnitPrice, GrossAmount, VATable, VATAmount, " & _
        ...
        Finally
            RecordActivity(UserID, ModuleID, Me.Name.ToString, "INSERT", "SP_No", txtVCECode.Text, BusinessType, BranchCode, "", activityStatus)
            SQL.FlushParams()
        End Try
    End Sub

    Private Sub LoadSP(ByVal TransID As String)
        Dim query As String
        query = " SELECT   TransID, SP_No, DateSP, VCECode, VCEName, ContractType, Size, Type, SignatoryName, SignatoryPosition, ProposalTitle, " & _
        ...
        If SQL.SQLDR.Read Then
            ...
            SPNo = SQL.SQLDR("SP_No").ToString
            txtTransNum.Text = SPNo
            dtpDocDate.Text = SQL.SQLDR("DateSP").ToString
    ...
    Protected Sub LoadSPDetails(ByVal TransID As String)
        dgvSoftware.Rows.Clear()
        dgvHardware.Rows.Clear()
        ...
        query = " SELECT    Type, ItemCode, Description, UOM, QTY, ISNULL(UnitPrice,0) AS UnitPrice, " & _
                ...
                " FROM      tblSP_Details " & _
                " WHERE     tblSP_Details.TransId = " & TransID & " " & _
                " ORDER BY  LineNum "
        SQL.ReadQuery(query)
        While SQL.SQLDR.Read
    ...
    Private Function RecordExist(ByVal Record As String) As Boolean
        Dim query As String
        query = " SELECT * FROM tblSP WHERE TransID ='" & Record & "' "
        SQL.ReadQuery(query)
        If SQL.SQLDR.Read Then
            Return True
        Else
    ...
    Private Sub SaveSP()
        Try
            ...
            insertSQL = " INSERT INTO " & _
                        " tblSP(TransID, SP_No, BranchCode, BusinessCode, DateSP, VCECode, VCEName, ContactNo, ContactPerson, Email, ProposalTitle, " & _
                        ...
                        "        TermsType, TermsDP, TermsRetention, TermsBalance, TermsPDC) " & _
                        " VALUES(@TransID, @SP_No, @BranchCode, @BusinessCode, @DateSP, @VCECode, @VCEName, @ContactNo, @ContactPerson, @Email, @ProposalTitle," & _
                        "        @Address, @Area, @Region, @Province, @City, @Type, @Size, @ContractType, @Remarks, @SignatoryName, @SignatoryPosition, " & _
                        "        @GrossAmount, @Discount, @VATAmount, @NetAmount, @VATable, @VATInclusive, " & _
                        "        @RecurFee, @RecurFeeAmt, @RecurFeeDate, @RecurFeePeriod, " & _
            ...
            If rbInstallment.Checked = True Then
                SQL.AddParam("@TermsType", "Installment")
                If IsNumeric(txtDP.Text) Then
                    SQL.AddParam("@TermsDP", CDec(txtDP.Text))
                Else
                    SQL.AddParam("@TermsDP", 0)
                End If

                If IsNumeric(txtRetention.Text) Then
                    SQL.AddParam("@TermsRetention", CDec(txtRetention.Text))
                Else
                    SQL.AddParam("@TermsRetention", 0)
                End If
    ...
    Private Sub ComputeTotal()
        ...
        Else
            txtNet.Text = CDec(CDec(txtGross.Text) - CDec(txtDiscount.Text) + CDec(txtVAT.Text)).ToString("N2")
        End If
    End Sub

    Public Sub LoadItem(ByVal SPNo As String)
        Try
            Dim query As String
            ...
            SQL.FlushParams()
            SQL.AddParam("@SoftwareCode", SPNo)
            SQL.AddParam("@Description", SPNo)
            SQL.AddParam("@Price", SPNo)
    ...
    Private Sub frmSP_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub tsbSearch_Click(sender As System.Object, e As System.EventArgs) Handles tsbSearch.Click
        If Not AllowAccess("SP_VIEW") Then
            msgRestricted()
        Else
    ...
    Private Sub tsbPrevious_Click(sender As System.Object, e As System.EventArgs) Handles tsbPrevious.Click
        If SPNo <> "" Then
            Dim query As String
            query = " SELECT Top 1 TransID FROM tblSP  WHERE SP_No < '" & SPNo & "' ORDER BY SP_No DESC "
            SQL.ReadQuery(query)
            If SQL.SQLDR.Read Then
                TransID = SQL.SQLDR("TransID").ToString
                LoadSP(TransID)
    ...
    Private Sub tsbNext_Click(sender As System.Object, e As System.EventArgs) Handles tsbNext.Click
        If SPNo <> "" Then
            Dim query As String
            query = " SELECT Top 1 TransID FROM tblSP  WHERE SP_No > '" & SPNo & "' ORDER BY SP_No ASC "
            SQL.ReadQuery(query)
            If SQL.SQLDR.Read Then
                TransID = SQL.SQLDR("TransID").ToString
                LoadSP(TransID)
