Public Class frmPJ
    ...
    Private Sub SavePJ()
        Try
            ...
            insertSQL = " INSERT INTO " & _
                        " tblPJ (TransID, PJ_No, VCECode, BranchCode, BusinessCode, DatePJ, TotalAmount, Remarks, LN_Ref, TransAuto, WhoCreated, MemTransfer) " & _
                        " VALUES(@TransID, @PJ_No, @VCECode, @BranchCode, @BusinessCode, @DatePJ, @TotalAmount, @Remarks, @LN_Ref, @TransAuto, @WhoCreated, @MemTransfer)"
            SQL.FlushParams()
    ...
    Private Sub LoadPJ(ByVal ID As String)
        Dim query As String
        query = " SELECT  TransID, PJ_No, tblPJ.VCECode, VCEName, DatePJ, TotalAmount, Remarks, ISNULL(LN_Ref,0) as LN_Ref, tblPJ.Status, MemTransfer  " & _
                " FROM    tblPJ LEFT JOIN tblVCE_Master " & _
                " ON      tblPJ.VCECode = tblVCE_Master.VCECode " & _
                " WHERE   TransID ='" & ID & "' "
        SQL.ReadQuery(query)
        If SQL.SQLDR.Read Then
    ...
    Private Sub LoadAccountingEntry(ByVal TransID As Integer)
        Try
            ...
            query = " SELECT JE_No, VCECode, VCEName, AccntCode, AccntTitle, Particulars, RefNo," & _
                    ...
                    " FROM   View_GL  " & _
                    " WHERE  RefType ='PJ' AND RefTransID ='" & TransID & "' "
            SQL.ReadQuery(query)
            dgvEntry.Rows.Clear()
                ...
                If txtStatus.Text <> "Cancelled" AndAlso MsgBox("Are you sure you want to cancel this record?", MsgBoxStyle.Exclamation + MsgBoxStyle.YesNo, "GR8 Message Alert") = MsgBoxResult.Yes Then
                    Try
                        ...
                        insertSQL = " INSERT INTO tblJE_Details(JE_No, AccntCode, VCECode, Debit, Credit, Particulars, RefNo, LineNumber, OtherRef) " & _
                                    ...
                                    " WHERE JE_No IN (SELECT DISTINCT JE_No FROM tblJE_Header WHERE RefType ='PJ' AND RefTransID ='" & TransID & "') "
                        SQL.ExecNonQuery(insertSQL)
                ...
                ElseIf txtStatus.Text = "Cancelled" AndAlso MsgBox("Are you sure you want to un-cancel this record?", MsgBoxStyle.Exclamation + MsgBoxStyle.YesNo, "GR8 Message Alert") = MsgBoxResult.Yes Then
                    Try
                        ...
                        insertSQL = " DELETE FROM tblJE_Details " & _
                                    " WHERE JE_No IN (SELECT DISTINCT JE_No FROM tblJE_Header WHERE RefType ='PJ' AND RefTransID ='" & TransID & "') AND LineNumber = 0 "
                        SQL.ExecNonQuery(insertSQL)
                        Msg("Record cancelled successfully", MsgBoxStyle.Information)
