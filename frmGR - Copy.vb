Public Class frmGR
    ...
    Private Sub LoadGR(ByVal ID As String)
        ...
        query = " SELECT   TransID, GR_No, VCECode, DateGR, Type, ReceivedBy, ReceivedFrom, WHSE_From, WHSE_To, Remarks, Status, GI_Ref " & _
                " FROM     tblGR " & _
                " WHERE    TransId = '" & ID & "' " & _
                " ORDER BY TransId "
        SQL.ReadQuery(query)
        If SQL.SQLDR.Read Then
            ...
            txtGIRef.Text = SQL.SQLDR("GI_Ref").ToString
            disableEvent = True
    ...
    Protected Sub LoadGRDetails(ByVal TransID As String)
        Dim ctr As Integer = 0
        Dim query As String
        query = " SELECT	tblGR_Details.ItemCode, tblGR_Details.Description, tblGR_Details.UOM, QTY, tblGR_Details.WHSE " & _
    ...
    Private Sub SaveGR()
        Try
            ...
            insertSQL = " INSERT INTO " & _
                        " tblGR(TransID, GR_No, BranchCode, BusinessCode, DateGR, VCECode, ReceivedBy, ReceivedFrom, WHSE_From, WHSE_To, Type, Remarks, GI_Ref, WhoCreated) " & _
                        " VALUES (@TransID, @GR_No, @BranchCode, @BusinessCode, @DateGR, @VCECode, @ReceivedBy, @ReceivedFrom, @WHSE_From, @WHSE_To, @Type, @Remarks, @GI_Ref, @WhoCreated) "
            SQL.FlushParams()
            ...
            SQL.AddParam("@GI_Ref", txtGIRef.Text)
            ...
            For Each row As DataGridViewRow In dgvItemMaster.Rows
                If Not row.Cells(dgcQTY.Index).Value Is Nothing AndAlso Not row.Cells(chItemCode.Index).Value Is Nothing Then
                    ...
                    SaveINVTY("OUT", ModuleID, "GI", MR_ID, dtpDocDate.Value.Date, ItemCode, WHSEfrom, QTY, UOM, UnitCost)
                    SaveINVTY("IN", ModuleID, "GR", TransID, dtpDocDate.Value.Date, ItemCode, WHSEto, QTY, UOM, UnitCost)
                End If
            Next
            ComputeWAUC("GI", MR_ID)
            ComputeWAUC("GR", TransID)

            Dim updateSQL As String
            updateSQL = " UPDATE tblGI SET Status ='Closed' WHERE TransID = '" & MR_ID & " '"
    ...
    Private Sub UpdateGR()
        Try
            ...
            updateSQL = " UPDATE    tblGR " & _
                        " SET       GR_No = @GR_No, BranchCode = @BranchCode, BusinessCode = @BusinessCode, DateGR = @DateGR, VCECode = @VCECode, GI_Ref = @GI_Ref, " & _
    ...
    Private Sub tsbNext_Click(sender As System.Object, e As System.EventArgs) Handles tsbNext.Click
        If GRNo <> "" Then
            ...
            SQL.ReadQuery(query)
            If SQL.SQLDR.Read Then
                GRNo = SQL.SQLDR("TransID").ToString
                LoadGR(TransID)
            Else
    ...
    Private Sub LoadGI(ByVal ID As String)
        Try
            ...
            If SQL.SQLDR.Read Then
                ...
                txtGIRef.Text = SQL.SQLDR("GI_No").ToString
                WHSEfrom = SQL.SQLDR("WHSE_From").ToString
                WHSEto = SQL.SQLDR("WHSE_To").ToString
                disableEvent = True
                ...
                query = " SELECT tblGI_Details.ItemCode, Description, UOM, IssueQTY, WHSE " & _
